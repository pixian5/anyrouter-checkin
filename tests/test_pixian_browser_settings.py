...
from pixian_overlay.utils import browser
...
@pytest.mark.asyncio
async def test_verify_browser_login_retries_transient_empty_profile(monkeypatch):
	class FakePage:
		url = 'https://agentrouter.org/console'
	...
	monkeypatch.setattr(browser, 'PROFILE_VERIFY_POLL_INTERVAL_SECONDS', 0.001)
	page = FakePage()

	profile = await verify_browser_login(
		cast(Any, page), 'https://agentrouter.org/console', timeout_ms=100, user_info_path='/api/user/self'
	)
