...
- 2026-08-18：0.4.0 部署后正式 systemd 复核中，两个 AgentRouter 账号各增加 $25 并写入审计；AnyRouter 在 00:10（08:00 日界线前）接口返回 success 但余额仍为 $7001.30 / $48.70，正确按未证明奖励失败并留待 08:00 重试。随后补强失败审计明细，保留这类响应实际观察到的余额和累计消耗，不再显示为空。
